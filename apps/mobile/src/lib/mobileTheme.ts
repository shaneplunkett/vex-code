...
export function getMobileThemeVariables(
  ...
  const baseVariables = (() => {
    if (themeId === DEFAULT_MOBILE_THEME_ID) return VEX_MOBILE_THEME_VARIABLES[appearance];
    const theme =
...
export function getMobileThemePreviewColors(
  ...
  if (themeId === DEFAULT_MOBILE_THEME_ID) return VEX_MOBILE_THEME_PREVIEW_COLORS[appearance];
