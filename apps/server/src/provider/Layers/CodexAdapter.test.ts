...
sessionErrorLayer("CodexAdapterLive session errors", (it) => {
  ...
  it.effect("keeps canonical skill syntax when sending a selected Codex skill", () =>
    Effect.gen(function* () {
      const adapter = yield* CodexAdapter;
      const threadId = asThreadId("thread-codex-skill");
      yield* adapter.startSession({
        provider: ProviderDriverKind.make("codex"),
        threadId,
        runtimeMode: "full-access",
      ...
      NodeAssert.deepStrictEqual(runtime.sendTurnImpl.mock.calls[0]?.[0], { input });
    }),
  );
  ...
  it.effect("uses T3CODE_CODEX_LAUNCH_ARGS for the session runtime", () => {
    ...
    const layer = Layer.effect(
      ...
      Effect.gen(function* () {
        ...
        return yield* makeCodexAdapter(codexConfig, {
          sessionEnvironment: makeProviderSessionEnvironmentWithResolver({
            processEnvironment: {},
