  ...
  it.effect("uses canonical source-tree icons for unpackaged development", () =>
    Effect.gen(function* () {
      ...
      const fileSystemLayer = FileSystem.layerNoop({
        exists: (path) => Effect.succeed(String(path).includes("/assets/vex/")),
      });
      ...
      assert.match(Option.getOrThrow(icons.ico), /assets\/vex\/vex-code-windows\.ico$/);
      assert.match(Option.getOrThrow(icons.png), /assets\/vex\/vex-code-universal-1024\.png$/);
